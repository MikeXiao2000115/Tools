...
namespace tools {
	namespace alg {
		...
		*****************************************************************************/
		template<class T, class vec = typename std::vector<T>, typename = std::enable_if_t<std::is_integral_v<T>&&std::is_unsigned_v<T>>>
		void NTT_convolution(vec& a, vec& b, T g, size_t c, size_t k) {
			...
			T mod = static_cast<T>((c << k) + 1);
			T alpha = mod::pow(mod)(g, c << (k - logn)), ialpha = mod::inv(mod)(alpha), Ninv = alg::mod::inv(mod)(len);
			auto mul = mod::mult(mod);
			auto mis = mod::minus(mod);
			auto add = mod::plus(mod);
			auto check = mod::check_u(mod);
			std::transform(a.cbegin(), a.cend(), a.begin(), check);
			std::transform(b.cbegin(), b.cend(), b.begin(), check);
			a.resize(len, 0); b.resize(len, 0);
			alg::RotationSumTransform_base2(a.begin(), alpha, logn, 1, 0, mul, mul, mis, add);
			alg::RotationSumTransform_base2(b.begin(), alpha, logn, 1, 0, mul, mul, mis, add);
			std::transform(a.cbegin(), a.cend(), b.cbegin(), a.begin(), [mul](auto a, auto b) {return mul(a, b); });
			alg::RotationSumTransform_base2(a.begin(), ialpha, logn, 1, 0, mul, mul, mis, add);
			std::transform(a.cbegin(), a.cend(), a.begin(), [Ninv, mul](auto c) {return mul(c, Ninv); });
			auto itnz = a.cbegin();
		...
		*****************************************************************************/
		template<class T = std::uint64_t, class vec = typename std::vector<T>, typename = std::enable_if_t<std::is_integral_v<T> && std::is_unsigned_v<T>>>
		void auto_ntt_convolution(vec& a, vec& b, T g = 5, size_t c = 3, size_t k = 30) {
			auto logn = log2n(a.size() + b.size() - 1);
			size_t len = 1ull << logn;

			if (a.size() * b.size() * .470 <= logn * len * 35.5) {
				vec c;
